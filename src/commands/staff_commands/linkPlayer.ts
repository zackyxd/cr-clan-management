  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (components && components.length > 0) {
        ...
        // Check if rename feature is enabled and add button if player_name exists
        ...
        if (renameEnabled.enabled && player_name) {
          // Add rename button
          const renameButton = new ButtonBuilder()
            .setCustomId(makeCustomId('b', 'link_rename', guild.id, { cooldown: 5, extra: [user.id, player_name] }))
            .setLabel('Rename User?')
            .setStyle(ButtonStyle.Primary);

          const row = new ActionRowBuilder<ButtonBuilder>().addComponents(renameButton);
          replyComponents.push(row);
        }

        await interaction.editReply({ embeds: [embed], components: replyComponents });
        await client.query('COMMIT');
      }
    } catch (error) {
