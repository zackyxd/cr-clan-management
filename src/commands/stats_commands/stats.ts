  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    try {
      ...
      if (subcommand === 'lineup-order') {
        ...
        for (let i = 0; i < resolvedOrder.length; i++) {
          // Lineups: protect # column, columns before/after Fame, and gap column.
          // Leave Tag (col 1) and Fame (col 5) unprotected so users can edit tags and filter/sort by fame.
          ...
          lineupsGrid.allRequests.push(
            ...
            // Protect columns 2-4 (before Fame)
            buildProtectedRangeRequest(
              lineupsSheetId,
              {
                ...lineupRowRange,
                startColumnIndex: lineupStartCol + 2,
                endColumnIndex: lineupStartCol + 5,
              },
              LINEUPS_PROTECTION_NOTE,
              protectedRangeEditors,
            ),
            // Protect gap column (col 6)
            buildProtectedRangeRequest(
              lineupsSheetId,
              {
                ...lineupRowRange,
                startColumnIndex: lineupStartCol + 6,
