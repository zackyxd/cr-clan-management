...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    if (successfullyAdded) {
      description = `Successfully added ${channel} to be used as the log channel for clan invites.`;
      embedColor = EmbedColor.SUCCESS;
    } else {
      description = `Error making ${channel} the log channel for clan invites.`;
